	...
	public class ReorderableListControl {
		...
		private bool DoRemoveButton(Rect position, bool visible) {
			...
			switch (Event.current.GetTypeForControl(controlID)) {
				case EventType.MouseDown:
                    ...
                    //Debug.Log("Down");
			        Rect inAudioArea = position;
			        inAudioArea.width = 50;
                    if (GUI.enabled && Event.current.button != 1 && inAudioArea.Contains(mousePosition))
                    {
		...
		private void DrawListItem(EventType eventType, Rect position, IReorderableListAdaptor adaptor, int itemIndex) {
			...
			if (eventType == EventType.Repaint && visible) {
				// Draw grab handle?
				if (draggable)
                    GUI.DrawTexture(new Rect(position.x + 6, position.y + position.height / 2f - 3, 9, 5), EditorResources.GrabHandle);

				// Draw splitter between list items.
				if (!_tracking || itemIndex != s_AnchorIndex)
                    GUI.DrawTexture(new Rect(position.x, position.y - 1, position.width, 1), EditorResources.ItemSplitter);
			}
		...
		private void DrawFloatingListItem(EventType eventType, IReorderableListAdaptor adaptor, float targetSlotPosition) {
			if (eventType == EventType.Repaint) {
				...
				GUI.DrawTexture(targetPosition, EditorResources.ItemSplitter);
				...
				if (Event.current.type != EventType.Used) {
					switch (eventType) {
						case EventType.MouseDown:
					        ...
					        Rect dragArea = itemPosition;
                            dragArea.width = 50; //InAudio fixed width as the whole area should not be dragable
					        if (GUI.enabled && dragArea.Contains(mousePosition))
					        {
					            //Debug.Log("Stolen");
